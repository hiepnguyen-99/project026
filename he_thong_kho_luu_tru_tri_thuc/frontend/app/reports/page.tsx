...
export default function Reports() {
  ...
  const actionChartValues = u.actions.slice(0,12).map(x=>Math.max(8,Math.min(100,x.count*10)));
  ...
  return <div><PageHeader eyebrow="Báo cáo và phân tích" title="Báo cáo kho tri thức" description="Theo dõi mức độ sử dụng và chất lượng kho tri thức." actions={<button className="btn-primary" onClick={exportReport}><Download size={15}/>Xuất báo cáo</button>}/>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2"><Panel title="Hoạt động theo loại" description="Từ audit log"><div className="p-5">{actionChartValues.length?<Bars values={actionChartValues}/>:<p className="muted py-16 text-center text-sm">No data available</p>}</div></Panel><Panel title="Chất lượng kho tri thức"><div className="p-5 space-y-4">{[["Tài liệu lỗi thời",q.stale.length],["Nhóm trùng lặp",q.duplicates.length],["Học phần thiếu tài liệu",q.missing_course_documents.length]].map(x=><div key={x[0]} className="flex justify-between rounded-lg border border-[var(--border)] p-3 text-xs"><strong>{x[0]}</strong><span className="badge badge-amber">{x[1]}</span></div>)}</div></Panel></div>
