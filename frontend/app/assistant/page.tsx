...
function Sources({ citations }: { citations: Citation[] }) {
  ...
  return <div className="mt-5 border-t border-[var(--border)] pt-4">
    ...
    <div className="grid gap-2 sm:grid-cols-2">{citations.map(item => <Link href={`/documents/${item.id}/preview`} key={item.id} className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3 transition hover:border-blue-300 hover:bg-blue-50/40">
